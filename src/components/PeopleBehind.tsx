...
const CTAButton: React.FC<{ label: string; onClick?: () => void }> = ({
  ...
  <button
    onClick={onClick}
    className="flex w-full md:w-fit items-center justify-center gap-3 rounded-full px-5 py-3 text-[14px] md:text-[16px] font-medium transition-all hover:opacity-90"
    style={{
      background: "#A8CFA1",
      color: "#2C3C44",
      fontFamily: "Giphurs",
...
const PhotoCaption: React.FC<{
  title: string;
  desc: string;
}> = ({ title, desc }) => (
  <figcaption className="absolute flex flex-col md:flex-row gap-6 md:bottom-4 bottom-12 md:left-4 md:right-4">
    <div
...
const PeopleBehind: React.FC<PeopleBehindProps> = ({
  ...
  return (
    <section className="w-full py-14 md:pl-[120px]">
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-center">
        <div className="max-w-xl px-6 md:px-0">
          ...
          <h2
            className="pt-4 text-[32px] md:text-[42px] font-semibold leading-12 md:leading-[61.6px] -tracking-[0.28px]"
            style={{
              color: "#2C3C44",
          ...
          <p
            className="pt-2 pb-6 text-[14px] md:text-[16px] font-normal md:leading-[25.6px] leading-[22.4px] text-left"
            style={{
              color: "#64737A",
              fontFamily: "Giphurs",
            }}
